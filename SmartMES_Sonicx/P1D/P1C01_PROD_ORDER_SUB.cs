...
namespace SmartMES_Sonicx
{
    public partial class P1C01_PROD_ORDER_SUB : Form
    {
        ...
        private void P1C01_PROD_ORDER_SUB_Load(object sender, EventArgs e)
        {
            ...
            {
                dtpPlanDate.Value = DateTime.Parse(parentWin.dataGridView1.Rows[parentRowIdx].Cells[0].Value.ToString());
                tbProduct.Tag = parentWin.dataGridView1.Rows[parentRowIdx].Cells[1].Value.ToString();
                tbProduct.Text = parentWin.dataGridView1.Rows[parentRowIdx].Cells[4].Value.ToString();
                tbModel.Text = parentWin.dataGridView1.Rows[parentRowIdx].Cells[5].Value.ToString();
                tbProdQty.Text = parentWin.dataGridView1.Rows[parentRowIdx].Cells[6].Value.ToString();
                tbContents.Text = parentWin.dataGridView1.Rows[parentRowIdx].Cells[8].Value.ToString();

                dtpPlanDate.Enabled = false;
                lbProd.Visible = false;
            }

            this.ActiveControl = tbProdQty;
        }
        ...
        private void Save()
        {
            ...
            else
            {
                sql = "update productionplan set ProdQty = " + sProdQty + ", Contents = '" + sBigo + "' " +
                    "where PlanDate = '" + sPlanDate + "' and productID = " + sProdID;
        ...
        private void ProdEventMethod(object sender)
        {
            ...
            tbModel.Text = sProd.Substring(sProd.IndexOf("#3/") + 3, sProd.Length - (sProd.IndexOf("#3/") + 3));
        }
    }
}
